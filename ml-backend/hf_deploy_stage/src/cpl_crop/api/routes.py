...
async def predict_v2(
    ...
    # Build confidence weights, then DROP the weight of any cross-check whose
    # component is disabled and RENORMALIZE the rest to sum to 1.0. Without
    # this, disabling the hierarchical bundle / CLIP left ~0.35 of the weight
    # pointing at always-zero signals, capping the fused score at ~0.65 — so a
    # 96%-confident prediction read as ~53%. (See confidence.py.)
    cw = {
        "quality": settings.confidence_weight_quality,
        "seg": settings.confidence_weight_seg,
        "area": settings.confidence_weight_area,
        "top1": settings.confidence_weight_top1,
        "gap": settings.confidence_weight_gap,
        "crop_router": settings.confidence_weight_crop_router if hierarchical is not None else 0.0,
        "per_crop_head": settings.confidence_weight_per_crop_head if hierarchical is not None else 0.0,
        "crop_agreement": settings.confidence_weight_crop_agreement if crop_verifier is not None else 0.0,
    }
    _wsum = sum(cw.values()) or 1.0
    cw = {k: v / _wsum for k, v in cw.items()}
    ...
    signals = fuse_confidence(
        ...
        weight_quality=cw["quality"],
        weight_seg=cw["seg"],
        weight_area=cw["area"],
        weight_top1=cw["top1"],
        weight_gap=cw["gap"],
        weight_crop_router=cw["crop_router"],
        weight_per_crop_head=cw["per_crop_head"],
        weight_crop_agreement=cw["crop_agreement"],
    )

    # Open-set leaf gate (CLIP): does this image even look like a crop leaf?
    # Run on the ORIGINAL image so non-leaves (keyboard/hand/wall) that fooled
    # the segmenter are still rejected here.
    is_crop_leaf = True
    clip_leaf_prob: float | None = None
    if crop_verifier is not None:
        try:
            clip_leaf_prob = crop_verifier.leaf_probability(image_rgb)
            is_crop_leaf = clip_leaf_prob >= settings.crop_verifier_leaf_min_prob
        except Exception as e:  # noqa: BLE001
            logger.warning("predictv2.leaf_gate_failed", error=f"{type(e).__name__}: {e}")
    ...
    routing = route(
        ...
        min_top1=settings.router_min_top1,
        is_crop_leaf=is_crop_leaf,
