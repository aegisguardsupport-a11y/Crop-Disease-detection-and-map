...
class Settings(BaseSettings):
    ...
    # Confidence engine weights (must sum to 1.0; we don't enforce, just document)
    confidence_weight_quality: float = Field(default=0.15, ge=0.0, le=1.0)
    confidence_weight_seg: float = Field(default=0.20, ge=0.0, le=1.0)
    confidence_weight_area: float = Field(default=0.10, ge=0.0, le=1.0)
